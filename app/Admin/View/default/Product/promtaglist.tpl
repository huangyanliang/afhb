...
<block name="main">
 <div class="pubmain">
 <h1>类别管理<div class="pull-right">{:btn(array('vals'=>'添加资料','size'=>3,'icon'=>'plus','scene'=>'primary','url'=>U('Product/promtagadd','sty='.$sty.'&tables='.$dshow['table'])))}</div></h1>
 <form name="pubserch" method="get" action=""> 
  <div class="search">
   类别名称：<input type="text" class="text" name="topic" placeholder="类别名称" style="width:150px;">
   &nbsp;所属子类：{:dropdown($infdata,0,'请选择子类','ctag')}
  ...
  <table width="99%" border="0" cellspacing="0" cellpadding="0" align="center" class="{:tabstyle()}">
   ...
   <tr class="maintr">
    ...
    <td align="left" valign="middle">{:gtopic('proctag',$obj['ctag'])}</td>
    ...
    <td align="center" valign="middle">{:btn(array('vals'=>'编辑','icon'=>'edit','tips'=>'点击编辑数据','url'=>U('Product/promtagmod','tables='.$dshow['table'].'&id='.$obj['Id'])))}</td>
   </tr>
   </volist>
   <tr>
