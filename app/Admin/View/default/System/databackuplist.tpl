  ...
  <table width="99%" border="0" cellspacing="0" cellpadding="0" align="center" class="{:tabstyle()}">
   ...
   <tr class="maintr">
    ...
    <td align="left" valign="middle">
      {:btn(array('vals'=>'下载','icon'=>'download','tips'=>'点击下载此备份文件','url'=>U('system/downdataup','downpath='.$obj['path'])))}
      <if condition="$datasys['isdel'] eq 1">
       <a href="javascript:void(0)" data-path="{$obj['path']}" class="deldataup">{:btn(array('vals'=>'删除','icon'=>'trash','tips'=>'点击删除该备份文件','scene'=>'danger'))}</a> 
      </if>
      <if condition="$datasys['isre'] eq 1">
       <a href="javascript:void(0)" data-path="{$obj['path']}" class="redataup">{:btn(array('vals'=>'还原','icon'=>'refresh','tips'=>'点击还原该份备份数据','scene'=>'primary'))}</a>
      </if>
    </td>
   </tr>
   </volist>
   <tr>
    <td height="35" colspan="4" align="left" valign="middle"><font class="text-warning">{:icon('warning-sign')} 提示，任何形式删除的数据都无法找回，请谨慎操作！</font> </td>
   </tr>
  </table>
 </div>
