  ...
  <form name="sysmod" method="post" action="" onSubmit="return sysck(document.sysmod)">
    <table width="100%" border="0" align="center" cellpadding="2" cellspacing="2" class="{:tabstyle()}">
      <tr>
        ...
        <td height="42" align="left">{:input(array('name'=>'metatitle','val'=>$data['metatitle'],'scene'=>'topic','tips'=>'( 应用程序名称，用在站点显示中 )'))}</td>
      </tr>
      <tr>
        <td width="132" height="32" align="right" valign="middle">　站点ICP备案号：</td>
        <td height="32">{:input(array('name'=>'icpnote','val'=>$data['icpnote'],'units'=>'ICP','tips'=>'(申请过程是免费的,没有ICP备案号表示该网站不合法) '))}</td>
      </tr>
      <tr>
        <td width="132" height="82" align="right" valign="middle">　描述信息：</td>
        <td height="82" align="left">{:input(array('name'=>'metades','type'=>'textarea','val'=>$data['metades']))}</td>
      </tr>
      <tr>
        <td width="132" height="82" align="right" valign="middle">　站点关键字：</td>
        <td height="82" align="left">{:input(array('name'=>'metakey','type'=>'textarea','val'=>$data['metakey']))}</td>
      </tr>
      <tr>
        <td width="132" height="82" align="right" valign="middle">　可放代码：</td>
        <td height="82" align="left">{:input(array('name'=>'sys_code','type'=>'textarea','val'=>$data['sys_code'],'place'=>'您可以把第三方的代码放在这里，例如：百度统计代码','tips'=>'(您可以把第三方的代码放在这里，例如：百度统计代码)'))}</td>
      </tr>
      <tr class="hide">
      ...
      <tr>
        ...
        <td height="82" align="left">{:input(array('name'=>'c_text','type'=>'textarea','val'=>$data['c_text'],'tips'=>'(若项目选择关闭，显示该属性内容)'))}</td>
      </tr>
      <tr>
        <td width="132" height="32" align="right" valign="middle">　后台分页值：</td>
        <td height="32" align="left">{:input(array('name'=>'adminpage','val'=>$data['adminpage'],'scene'=>'ord','tips'=>'(设置后台资料管理分页值，大于1的整数，清除缓存后生效)'))}</td>
      </tr>
      <tr>
