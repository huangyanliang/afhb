...
<block name="main">
 ...
 <h1>添加管理员</h1>
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" class="{:tabstyle()}">
   <tr>
    ...
    <td height="32" align="left">{:input(array('name'=>'user','icon'=>'user','width'=>18,'add'=>'ckuser','place'=>'登录ID','tips'=>'必填'))}</td>
   </tr>
   <tr>
     <td height="32" align="right" valign="middle">姓名：</td>
     <td height="32" align="left">{:input(array('name'=>'name','icon'=>'user','width'=>18,'place'=>'姓名'))}</td>
   </tr>
   <tr>
   ...
   <tr>
     ...
     <td height="32" align="left">{:input(array('type'=>'password','name'=>'pass','icon'=>'log-out','width'=>30,'place'=>'登录密码','tips'=>'长度大于6位，支持大小写'))}</td>
   </tr>
   <tr>
     <td height="32" align="right" valign="middle">重复密码：</td>
     <td height="32" align="left">{:input(array('type'=>'password','name'=>'notpass','icon'=>'log-out','place'=>'重复密码','width'=>30,'tips'=>'长度大于6位，支持大小写'))}</td>
   </tr>
   <tr>
 ...
 <script type="text/javascript">
  ...
  $(".ckuser").on("blur",function(e){
	...
	if(user!=''){
	   $.post('{:U("Admin/ckuser")}',{'user':user},function(data){
		  if(data==1){
